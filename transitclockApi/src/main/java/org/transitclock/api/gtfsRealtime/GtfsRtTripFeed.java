...
public class GtfsRtTripFeed {
	...
	private TripUpdate createTripUpdate(List<IpcPrediction> predsForTrip) {
		...
		// To determine whether to include the vehicle in the TripUpdate
		boolean isVehicleOnTrip = false;

		// Based on the first prediction determine the current IpcVehicle info
		IpcPrediction firstPred = predsForTrip.get(0);
		IpcVehicleGtfsRealtime vehicle = getVehicle(firstPred.getVehicleId());
		...
		if (firstPred.getTripId() != null) {
			...
			// Check if the vehicle is on this particular trip
			// since predictions for future trips can be generated by a vehicle that's not on that trip yet
			if (!firstPred.getTripId().equals("") && vehicle != null && vehicle.getTripId().equals(firstPred.getTripId())) {
...
//		  tripUpdate.setDelay(firstPred.getDelay()); // set schedule deviation
		...
		// Add the VehicleDescriptor information if the vehicle is currently on the trip
		if (isVehicleOnTrip) {
		...
		for (IpcPrediction pred : predsForTrip) {
			...
			// Set the relation between this StopTime and the static schedule.
			// Possible values are SCHEDULED, SKIPPED and NO_DATA. Since we are
			// only creating StopTimeUpdate messages based on the predictions we
			// have, and based on the static GTFS schedule, the relationship will
			// always be SCHEDULED.
	...
	private Map<String, List<IpcPrediction>> getPredictionsPerTrip() {
		...
		// Sort the predictions by stop sequence
		for (List<IpcPrediction> predsForTrip : predictionsByTrip.values()) {
			predsForTrip.sort(Comparator.comparingInt(IpcPrediction::getGtfsStopSeq));
		}
	...
	/**
	 * Returns vehicle by vehicleId obtained via RMI.
	 * Returns null if there was a problem getting the data via RMI
	 * 
	 * @param vehicleId
	 * 
	 * @return Vehicle object, or null if not available.
	 */
	private IpcVehicleGtfsRealtime getVehicle(String vehicleId) {
		VehiclesInterface vehiclesInterface =
				VehiclesInterfaceFactory.get(agencyId);
		IpcVehicleGtfsRealtime vehicle = null;
		try {
			vehicle = (IpcVehicleGtfsRealtime) vehiclesInterface.get(vehicleId);
			logger.debug("Got vehicle {} from RMI", vehicleId);
			if (vehicle != null) {
				logger.debug("Vehicle tripId = {}", vehicle.getTripId());
			} else {
				logger.debug("Vehicle is null");
			}
		} catch (RemoteException e) {
			logger.error("Exception when getting vehicle {} from RMI", vehicleId, e);
		}
		return vehicle;
	}
